...
def result_analysis_shift_sub_keyword(positive_result, utt2keyword_idx, negative_result, test_result_dir, analysis_id, sub_keyword_list, sub_keyword_id):
    pos_hyp = []
    pos_ref = []
    #one_word_py = ''.join([ i[0] for i in pinyin(one_word, style=Style.FIRST_LETTER) ])
    for utt, result in positive_result.items():
        target_keyword_idx = utt2keyword_idx[utt]
        if target_keyword_idx not in sub_keyword_list:
        #if target_keyword_idx != one_word_idx:
            continue
        utt_pos_hyp = []
        utt_pos_ref = []
    ...
    for utt, result in negative_result.items():
        for chunk in result:
            for one_word_id in sub_keyword_list:
                chunk_target_score = chunk[one_word_id]
                if count_top1_keyword:
                    if chunk_target_score != max(chunk):
                        chunk_target_score = 0
                neg_hyp.append(chunk_target_score)
    ...
    plot_roc_curve(pos_ref+neg_ref, pos_hyp+neg_hyp, test_result_dir, "{}-{}".format(analysis_id, sub_keyword_id))
    if dcf_config is not None:
        compute_dcf(pos_ref+neg_ref, pos_hyp+neg_hyp, cost_miss, cost_fa, prior_target, test_result_dir, "{}-{}".format(analysis_id, sub_keyword_id))
...
if __name__ == '__main__':
    ...
    if plot_each_word:
        ...
        #    print(keyword, keywordidx)
            sub_keyword_list = [1, 2, 3, 7, 10, 35, 44]
            sub_keyword_id = "7zh"
            result_analysis_shift_sub_keyword(positive_result, utt2keyword_idx, negative_result, test_result_dir, analysis_id, sub_keyword_list, sub_keyword_id)
