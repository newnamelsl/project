...
def inference_debug(model, test_list, keyword_material, idx2keyword, keyword2idx):
    keyword, keyword_len = keyword_material
    num_keyword = keyword.size(0)
    result = {}
    utt2keyword_idx = {}
    for i, obj in enumerate(test_list):
        ...
        #hyp = [id2word[h] for h in hyp if h in id2word]
        det_result = det_result.view(-1).to('cpu').tolist()
        result.update({key: [det_result, hyp]})
        #result.update({key: det_result})
    return result, utt2keyword_idx
...
if __name__ == '__main__':
    ...
    result_path = os.path.join(test_result_dir, 'test.result.pt')
    meta_path = os.path.join(test_result_dir, 'fenyinta_meta.pt')
    ...
    positive_result_path = os.path.join(test_result_dir, 'test.result_pos.pt')
    ...
    #else:
    positive_result, utt2keyword_idx = inference_debug(model, positive_list, (keyword, keyword_len), idx2keyword, keyword2idx)
