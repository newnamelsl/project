...
def inference_shift_batch(model, test_list, keyword_material, idx2keyword, keyword2idx, shift=False, chunk_size=chunk_size):
    ...
    for i, obj in enumerate(test_list):
        ...
        while j * chunk_shift + chunk_size <= fbank.size(1):
            ...
            if cross_batch_len == 0:
                chunk_batch = copy.deepcopy(chunk)
                chunk_len_batch = copy.deepcopy(chunk_len)
                keyword_embedding_batch = copy.deepcopy(keyword_embedding)
                keyword_mask_batch = copy.deepcopy(keyword_mask)
            else:
                chunk_batch = torch.cat((chunk_batch, chunk), dim=0)
                chunk_len_batch = torch.cat((chunk_len_batch, chunk_len), dim=0)
                keyword_embedding_batch = torch.cat((keyword_embedding_batch, keyword_embedding), dim=0)
                keyword_mask_batch = torch.cat((keyword_mask_batch, keyword_mask), dim=0)
            cross_batch_len += 1
            if cross_batch_len >= cross_batch_size:
                ...
                speech_embedding_batch = speech_embedding_batch.repeat_interleave(num_keyword, dim=0)
                speech_mask_batch = speech_mask_batch.repeat_interleave(num_keyword, dim=0)
                # # speech_embedding = speech_embedding.repeat(num_keyword, 1, 1)
                # # speech_mask = speech_mask.repeat(num_keyword, 1, 1)
                ...
                #     speech_embedding_batch = copy.deepcopy(speech_embedding)
                #     speech_mask_batch = copy.deepcopy(speech_mask)
                #     keyword_embedding_batch = copy.deepcopy(keyword_embedding)
                #     keyword_mask_batch = copy.deepcopy(keyword_mask)
                # else:
                #     # chunk_batch = torch.cat((chunk_batch, chunk), dim=0)
                ...
                #     speech_embedding_batch = torch.cat((speech_embedding_batch, speech_embedding), dim=0)
                #     speech_mask_batch = torch.cat((speech_mask_batch, speech_mask), dim=0)
                #     keyword_embedding_batch = torch.cat((keyword_embedding_batch, keyword_embedding), dim=0)
                #     keyword_mask_batch = torch.cat((keyword_mask_batch, keyword_mask), dim=0)
                # cross_batch_len += 1
                # if cross_batch_len >= cross_batch_size:
                # input_data_batch = (chunk_batch, chunk_len_batch, keyword_batch, keyword_len_batch)
                cross_input_batch = (speech_embedding_batch, speech_mask_batch, keyword_embedding_batch, keyword_mask_batch)
