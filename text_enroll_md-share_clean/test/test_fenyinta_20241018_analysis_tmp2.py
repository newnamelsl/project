...
def inference_shift_batch(model, test_list, keyword_material, idx2keyword, keyword2idx, shift=False, chunk_size=98):
    keyword, keyword_len = keyword_material
    num_keyword = keyword.size(0)
    result = {}
    utt2keyword_idx = {}
...
def result_analysis_shift_one_word(positive_result, utt2keyword_idx, negative_result, test_result_dir, analysis_id, one_word, one_word_idx):
    ...
    for utt, result in positive_result.items():
        ...
        for chunk in result:
            chunk_target_score = chunk[target_keyword_idx]
            print("{}".format(chunk_target_score))
            if count_top1_keyword:
                if chunk_target_score != max(chunk):
                    chunk_target_score = 0
            utt_pos_hyp.append(chunk_target_score)
...
#        # 直接使用
#        make_fenyinta()
    keyword2phn = make_dict_from_file('temp_trans/bad/keyword2phnseq')
    #keyword2phn = make_dict_from_file(testset_dir+'/keyword2phnseq')
    phn2id = make_dict_from_file('resource/aishell2_kespeech_mandarin/cut_head_100_1channel/phone2id.txt')
    #phn2id = make_dict_from_file(train_data_dir+'/phone2id.txt')
...
#    test_material = torch.load(meta_path)
    model = load_model()
    keyword2id = {k: [int(vv) for vv in v] for k, v in keyword2phnid.items()}
    ...
    positive_list = read_data_list(os.path.join('temp_trans/bad/positive.datalist.txt'))
...
#    else:
    ...
    torch.save(positive_test_result, "temp_trans/bad/pos_result.pt")
