# pos segment, neg shift, pos 互相, keyword独立
# pos segment, neg shift, 无pos互相, keyword独立
# pos shift max, neg shift, 无pos互相, keyword独立
# pos shift max, neg shift, 无pos互相, keyword max
# pos segment, neg shift, 无pos互相, keyword max
...
#output_root = "temp_test_7zh_samp2_3_only_pos_top1"
#output_root = "temp_test_7zh_samp2_2"
#output_root = "temp_test_7zh_samp2"
...
pos_shift = False
neg_shift = True
#use_pos_pos_trials = False
use_pos_pos_trials = True
#count_top1_keyword = True
count_top1_keyword = False
#count_best_chunk = True
...
def print_keyword_score(det_result):
    for n, score in enumerate(det_result):
        if n == 0:
            f_keyword_score.write("\n{}: {}".format(idx2keyword[n], score))
        else:
            f_keyword_score.write(", {}: {}".format(idx2keyword[n], score))

def print_keyword_score_max(result):
    
    result_chunk_max = [0.0] * 7
    for chunk in result:
        print("chunk: {}".format(chunk))
        for i in range(len(chunk)):
            if chunk[i] > result_chunk_max[i]:
                result_chunk_max[i] = chunk[i]
    print("result_chunk_max: {}".format(result_chunk_max))
    exit()
    #for n, score in enumerate(det_result):
    #    if n == 0:
    #        f_keyword_score.write("\n{}: {}".format(idx2keyword[n], score))
...
def inference(model, test_list, keyword_material, idx2keyword, keyword2idx):
    ...
    for i, obj in enumerate(test_list):
        ...
        print_keyword_score(det_result)
...
def print_pos_hyp_best_chunk(utt, utt_pos_hyp, file):
    pos_hyp_dict = {}
    pos_hyp_dict["uttid"] = utt
    pos_hyp_dict["hyp"] = utt_pos_hyp
    pos_hyp_dict["hyp_best_chunk"] = max(utt_pos_hyp)
    file.write(json.dumps(pos_hyp_dict)+'\n')
...
def plot_roc_shift(positive_result, utt2keyword_idx, negative_result, output_root, analysis_id):
    ...
    for utt, result in positive_result.items():
        ...
        #print_pos_hyp_best_chunk(utt, utt_pos_hyp, file)
        if use_pos_pos_trials:
...
if __name__ == '__main__':
    ...
    #file = open('test_pos_hyp_no_shift.txt', 'w')
    #file = open('test_pos_hyp_best_chunk.txt', 'w')
    if os.path.exists(result_path):
        ...
        #f_keyword_score = open('test_keyword_score.txt', 'w')
