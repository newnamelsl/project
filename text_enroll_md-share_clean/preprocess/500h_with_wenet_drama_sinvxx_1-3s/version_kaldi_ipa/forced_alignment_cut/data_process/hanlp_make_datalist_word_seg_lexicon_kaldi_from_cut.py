...
def get_wav_text_from_cut_json(cut_json, cut_data_root):
    with open(cut_json) as f_cut:
        cut_dict = json.load(f_cut)
    text_dict = {}
    wav_dict = {}
    for uttid in cut_dict:
        for seg_dict in cut_dict[uttid]:
            #print(uttid, i)
            cut_id = re.sub("\.wav$", "", seg_dict["filename"])
            cut_wav_path = os.path.join(cut_data_root, seg_dict["file_path"])
            cut_text = seg_dict["text_segment"]
            wav_dict[cut_id] = cut_wav_path
            text_dict[cut_id] = cut_text
    return wav_dict, text_dict


def main(cut_json, cut_data_root, in_word2id, in_phone2id, in_lexicon, user_dict, out_word2id, out_phone2id, out_datalist, out_lexicon):
    wav_scp, text_scp = get_wav_text_from_cut_json(cut_json, cut_data_root)
...
if __name__ == '__main__':
    ...
    main(cut_json, cut_data_root, in_word2id, in_phone2id, in_lexicon, user_dict, out_word2id, out_phone2id, out_datalist, out_lexicon)
