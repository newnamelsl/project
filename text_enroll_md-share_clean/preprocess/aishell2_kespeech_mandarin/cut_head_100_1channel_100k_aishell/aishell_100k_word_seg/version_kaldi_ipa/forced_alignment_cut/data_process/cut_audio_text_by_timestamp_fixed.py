...
class AudioSegmentationToolFixed:
    ...
    def _warmup_librosa(self):
        ...
        try:
            ...
            # 进行一次实际的音频加载测试（如果有可用的音频文件）
            if self.datalist:
                first_key = next(iter(self.datalist))
                first_audio_path = self.datalist[first_key]['sph']
                if os.path.exists(first_audio_path):
                    _ = librosa.load(first_audio_path, sr=None, duration=0.1)  # 只加载0.1秒
            
            warmup_time = time.time() - start_time
            print(f"librosa预热完成，耗时: {warmup_time:.2f}秒")
...
def main():
    ...
    # 创建切割工具（包含librosa预热）
    tool = AudioSegmentationToolFixed(datalist_path, timestamp_path, text_path, output_dir)
