...
class AudioSegmentationToolMemoryFirst:
    def __init__(self, datalist_path: str, timestamp_path: str, text_path: str = None, output_dir: str = "output_segments"):
        ...
        # 内存中存储所有音频片段
        self.audio_segments = []  # 存储 (audio_data, sr, output_path) 的列表
    ...
    def cut_audio_to_memory(self, audio_key: str, threshold: int = 6, 
    ...
    def write_all_audio_segments(self, max_workers: int = 8):
        """批量写入所有音频片段"""
        if not self.audio_segments:
            print("没有音频片段需要写入")
            return
        
        print(f"开始写入 {len(self.audio_segments)} 个音频片段...")
        start_time = time.time()
        
        def write_single_segment(segment):
            try:
                sf.write(segment['output_path'], segment['audio_data'], segment['sr'])
                return True
            except Exception as e:
                print(f"写入失败 {segment['output_path']}: {e}")
                return False
        
        # 使用线程池并行写入
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(write_single_segment, segment) for segment in self.audio_segments]
            success_count = sum(1 for future in futures if future.result())
        
        write_time = time.time() - start_time
        print(f"音频片段写入完成: {success_count}/{len(self.audio_segments)} 个文件")
        print(f"写入耗时: {write_time:.2f}秒")
        print(f"写入速度: {success_count/write_time:.1f} 文件/秒")
    ...
    def cut_all_audio(self, threshold: int = 6, extend_before_ms: int = 0, extend_after_ms: int = 0) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for idx, audio_key in enumerate(self.datalist.keys()):
            if idx % 100 == 0:
                print(f"处理进度: {idx}/{total_files} ({idx/total_files*100:.1f}%)")
            
            try:
                cut_info = self.cut_audio_to_memory(audio_key, threshold, extend_before_ms, extend_after_ms)
        ...
        print(f"音频处理完成，耗时: {processing_time:.2f}秒")
        print(f"处理速度: {total_files/processing_time:.1f} 文件/秒")
        
        # 第二阶段：批量写入所有音频片段
        print("\n第二阶段：批量写入音频文件...")
        self.write_all_audio_segments()
...
def main():
    ...
    print(f"  策略: 先处理到内存，再批量写入")
    
    # 创建切割工具
    tool = AudioSegmentationToolMemoryFirst(datalist_path, timestamp_path, text_path, output_dir)
    ...
    if successful_audio > 0:
        print(f"平均处理速度: {successful_audio/total_time:.2f} 文件/秒")
