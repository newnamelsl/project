...
class AudioSegmentationToolMetadataOnly:
    ...
    def generate_cut_metadata(self, audio_key: str, threshold: int = 6, 
        ...
        try:
            ...
            print(f"错误: 无法读取音频文件信息 {audio_path}: {e}")
            return []
        ...
        # 生成切割元数据
        cut_info = []
        audio_duration_ms = total_frames / sr * 1000
        ...
        for i, ((start_word, end_word), (start_time, end_time)) in enumerate(zip(word_groups, segment_timestamps)):
            ...
            if start_sample < total_frames and end_sample <= total_frames:
                # 生成输出路径
    ...
    def generate_all_metadata(self, threshold: int = 6, extend_before_ms: int = 0, extend_after_ms: int = 0) -> Dict[str, List[Dict[str, Any]]]:
        """生成所有音频的切割元数据"""
        all_cut_info = {}
        ...
        for idx, audio_key in enumerate(self.datalist.keys()):
            if idx % 100 == 0:
                print(f"元数据生成进度: {idx}/{total_files} ({idx/total_files*100:.1f}%)")
            
            try:
                cut_info = self.generate_cut_metadata(audio_key, threshold, extend_before_ms, extend_after_ms)
                all_cut_info[audio_key] = cut_info
            except Exception as e:
               print(f"  元数据生成失败 {audio_key}: {e}")
               all_cut_info[audio_key] = []
        
        return all_cut_info
    
    def extract_segments_from_metadata(self, cut_info: Dict[str, List[Dict[str, Any]]], 
                                     target_segments: List[str] = None):
        """根据元数据提取指定的音频片段"""
        if target_segments is None:
            # 提取所有片段
            target_segments = []
            for segments in cut_info.values():
                for segment in segments:
                    target_segments.append(segment['filename'].replace('.wav', ''))
        
        print(f"根据元数据提取 {len(target_segments)} 个音频片段...")
        
        # 按源音频文件分组，减少重复加载
        source_groups = {}
        for audio_key, segments in cut_info.items():
            for segment in segments:
                segment_name = segment['filename'].replace('.wav', '')
                if segment_name in target_segments:
                    source_file = segment['source_audio']
                    if source_file not in source_groups:
                        source_groups[source_file] = []
                    source_groups[source_file].append(segment)
        
        extracted_count = 0
        for source_file, segments in source_groups.items():
            try:
                # 加载源音频文件一次
                ...
                for segment in segments:
                    ...
                    if start_sample < len(audio) and end_sample <= len(audio):
                        ...
                        sf.write(segment['file_path'], audio_segment, sr)
                        extracted_count += 1
                        
            except Exception as e:
                print(f"提取失败 {source_file}: {e}")
        
        print(f"成功提取 {extracted_count} 个音频片段")
...
def main():
    ...
    if len(sys.argv) < 6:
        print("用法:")
        print("  生成元数据: python script.py datalist timestamp text output threshold [extend_before] [extend_after]")
        print("  提取音频: python script.py datalist timestamp text output threshold [extend_before] [extend_after] --extract")
        sys.exit(1)
    ...
    extract_audio = '--extract' in sys.argv
    ...
    print(f"  模式: {'生成元数据+提取音频' if extract_audio else '仅生成元数据'}")
    
    # 创建切割工具
    tool = AudioSegmentationToolMetadataOnly(datalist_path, timestamp_path, text_path, output_dir)
    
    # 生成元数据
    print("\n开始生成切割元数据...")
    start_time = time.time()
    all_cut_info = tool.generate_all_metadata(threshold, extend_before_ms, extend_after_ms)
    ...
    print(f"\n元数据生成完成!")
    print(f"元数据生成耗时: {metadata_time:.2f}秒")
    print(f"成功处理音频数: {successful_audio}")
    print(f"总切割片段数: {total_segments}")
    if successful_audio > 0:
        print(f"元数据生成速度: {successful_audio/metadata_time:.2f} 文件/秒")
    
    # 可选：提取音频文件
    if extract_audio:
        print("\n开始提取音频文件...")
        extract_start = time.time()
        tool.extract_segments_from_metadata(all_cut_info)
        extract_time = time.time() - extract_start
        print(f"音频提取耗时: {extract_time:.2f}秒")
        print(f"总耗时: {metadata_time + extract_time:.2f}秒")
